...
import os
import logging
LOG_CDIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.dirname(LOG_CDIR)
DEBUG = True  # 标记是否在开发环境
LOGSDIR = "%s/logs"%BASE_DIR
...
LOGGING = {
    ...
    'formatters': {
        ...
        'standard': {
            ...
            'format': '[%(name)s:%(levelname)s(%(lineno)d)] [%(module)s:%(funcName)s]:%(message)s'
        }
    },
    ...
    'handlers': {
        ...
        'log': {
            ...
            'formatter': 'standard',
            'filename': '/tmp/opsagent.log',  # 输出位置
